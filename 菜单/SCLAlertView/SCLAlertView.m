...
- (void)viewWillLayoutSubviews
{
    ...
    kuandu=[UIScreen mainScreen].bounds.size.width;
    gaodu=[UIScreen mainScreen].bounds.size.height;
    float stw=self.view.frame.size.width;
    float sth=self.view.frame.size.height;
    
    scrollView.contentSize=CGSizeMake(self.view.frame.size.width, _buttons.count*50+70);
    if (self.view.frame.size.height>gaodu) {
        self.view.frame=CGRectMake(self.view.frame.origin.x, 50, stw, gaodu-100);
    }
    scrollView.frame=self.view.bounds;
    
    NSLog(@"_contentView.frame=%@",self.view);
    //横屏
